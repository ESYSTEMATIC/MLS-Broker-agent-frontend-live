<template>
    <div>
      ...
      >
        ...
        <p class="">
          {{ $t("TITLES.your_contacts_history") }}
        </p>
      </div>
      ...
      <div class="rounded-2xl light:bg-white card-shadow px-6 py-4">
        ...
        <div>
          <ProfileContactsTable v-if="pending" loading :items="[]" />
    
          <ProfileContactsTable v-else :items="[]" />
        </div>
      </div>
    </div>
  </template>
  ...
  const dataFilter = [
    {
      ...
      type: "search",
      key: "keyword",
      placeholder: t("LABELS.search") + " " + t("TITLES.contacts"),
    },
    {
      name: "email",
      type: "search",
