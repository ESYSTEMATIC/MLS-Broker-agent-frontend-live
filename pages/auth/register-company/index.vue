<template>
  <div
    class="relative max-h-[95vh] w-full overflow-y-auto overflow-x-hidden rounded-[24px]"
  >
    <div v-if="loadingAllData">
      <div
        class="g-white mx-auto bg-white p-3 dark:bg-[#0B0909] md:p-7 lg:w-[1200px]"
        v-if="showSteps"
      >
        <div
          class="mb-5 grid w-full grid-cols-12 items-center font-medium xl:mb-7"
        >
          <button
            type="button"
          ...
          <form class="grid grid-cols-12 gap-5">
            <div class="col-span-12 xl:col-span-6">
              <InputsBaseInput
                id="name"
                name="name"
                :label="`${$t('GLOBALS.company')} ${$t('LABELS.name')}`"
                :placeholder="`${$t('GLOBALS.company')} ${$t('INPUTS.namePlaceholder')}`"
                isChar
              />
            </div>
            ...
            <div class="col-span-12 xl:col-span-6">
              <InputsBaseInput
                id="office_address"
                name="office_address"
                :label="`${$t('GLOBALS.company')} ${$t('LABELS.address')}`"
                :placeholder="`${$t('GLOBALS.company')} ${$t(
                  'INPUTS.office_addressPlaceholder',
                )}`"
              />
            </div>
            ...
            <div class="col-span-12 xl:col-span-6">
              <InputsBaseInput
                ...
                isNumber
                :label="`${$t('GLOBALS.company')} ${$t('LABELS.phoneNumber')}`"
                :placeholder="`${$t('GLOBALS.company')} ${$t(
                  'INPUTS.phoneNumberPlaceholder',
                )}`"
              />
            </div>
            ...
            <div class="col-span-12 xl:col-span-6">
              <InputsBaseInput
                id="email"
                name="email"
                :label="`${$t('GLOBALS.company')} ${$t('LABELS.email')}`"
                :placeholder="`${$t('GLOBALS.company')} ${$t('INPUTS.emailPlaceholder')}`"
              />
            </div>
            ...
            <div class="col-span-12 xl:col-span-6">
              <InputsBaseInput
                ...
                name="office_tax_id"
                isNumber
                :label="`${$t('GLOBALS.company')} ${$t('LABELS.taxId')}`"
                :placeholder="`${$t('GLOBALS.company')} ${$t('INPUTS.taxIdPlaceholder')}`"
              />
            </div>

            <div class="col-span-12 xl:col-span-6">
              <InputsBaseInput
                id="office_commercial_registration"
                name="office_commercial_registration"
                isNumber
                :label="$t('LABELS.companyRegistrationNumber')"
                :placeholder="$t('INPUTS.companyRegistrationNumberPlaceholder')"
              />
            </div>
          ...
          <form class="grid grid-cols-12 gap-5">
            ...
            <div class="col-span-12">
              <FieldArray name="branches" v-slot="{ fields, push, remove }">
                <div
                  class="mb-5 grid grid-cols-12 gap-x-5 gap-y-3 last-of-type:mb-0"
                  v-for="(_, i) in fields"
                  :key="i"
                >
                  ...
                  <div class="col-span-12 xl:col-span-4">
                    <InputsBaseInput
                      ...
                      isChar
                      :label="$t('LABELS.branchWebsite')"
                      :placeholder="$t('INPUTS.branchWebsitePlaceholder')"
                    />
                  </div>

                  <div
                    class="col-span-12 flex flex-col items-start justify-between lg:flex-row lg:items-center xl:col-span-4"
                  >
                    <div>
                      <h5 class="mb-3 text-title-light dark:text-title-light">
                        {{ $t("LABELS.isMaster") }}
                      </h5>
                      ...
                      <div class="flex items-center gap-5">
                        <div class="flex items-center gap-1.5">
                          <input
                            ...
                            v-model="masters[i].is_master"
                          />

                          <label
                            :for="`yesIsMaster${i}`"
                            class="cursor-pointer text-text-light dark:text-text-dark"
                            >{{ $t("INPUTS.yes") }}</label
                          >
                        </div>
                    ...
                    <div class="flex items-center gap-3">
                      <button
                        type="button"
                        class="flex size-[30px] items-center justify-center rounded-full bg-box-container-light text-sm text-green-500 dark:bg-box-container-dark dark:text-text-dark"
                        @click="
                          push({
                            name: '',
                            website: '',
                          });
                          masters.push({ is_master: 0 });
                        "
                      >
                      ...
                      <button
                        v-if="i !== 0"
                        type="button"
                        class="flex size-[30px] items-center justify-center rounded-full bg-primary text-sm text-white"
                        @click="
...
also this content
          ...
          >
            <form>
              ...
              <div class="mb-5">
                ...
                <img
                  v-else
                  :src="initialValues.signatureAsImg"
                  alt="signature"
                />
              </div>
      ...
      >
        ...
        <p class="pb-5 text-center text-text-dark dark:text-text-light">
...
const roleOptions = ref([
  {
    ...
    name: i18n.t("LABELS.brokerageCompany"),
  },
  {
    id: "developer_company",
    name: i18n.t("LABELS.developerCompany"),
  },
]);
...
const developerSchema = yup.object().shape({
  ...
  office_address: yup.string().required(i18n.t("ERRORS.office_address")),
  office_phone_number: yup
    .string()
    .required(i18n.t("ERRORS.office_phone_number")),
  email: yup
    .string()
  ...
  office_tax_id: yup.string().required(i18n.t("ERRORS.office_tax_id")),
  office_commercial_registration: yup
    .string()
    .required(i18n.t("ERRORS.office_commercial_registration")),
...
async function handleDeveloperInfo(values) {
  ...
  developerInitialValues.value = {
    ...
    office_commercial_registration: values.office_commercial_registration,
...
async function handlePrinciplesDeveloper(values) {
  ...
  principlesInitialValues.value = values;
...
async function handleSignature(values) {
  ...
  frmData.append("company_type", allValues.value.role);
  frmData.append("name", allValues.value.name);
  frmData.append("address", allValues.value.office_address);
  frmData.append("phone", allValues.value.office_phone_number);
  frmData.append("email", allValues.value.email);
  frmData.append("website", allValues.value.website);
  frmData.append("tax_id", allValues.value.office_tax_id);
  frmData.append(
    "commercial_registration",
    allValues.value.office_commercial_registration,
  );
  frmData.append("password", allValues.value.password);
  // =================================================================
  ...
  allValues.value.branches.map((branch, i) => {
    frmData.append(`branches[${i}][name]`, branch.name);
    frmData.append(`branches[${i}][address]`, branch.address);
    frmData.append(
      `branches[${i}][is_master]`,
      masters.value[i].is_master ? 1 : 0,
    );
  });
  // =================================================================
  frmData.append(
  ...
  frmData.append("signature_full_name", signature.value);
  ...
  await $fetch("/register-company", {
    ...
    .catch((e) => {
      ...
      toast.error(e.response._data.message[0]);
    })
